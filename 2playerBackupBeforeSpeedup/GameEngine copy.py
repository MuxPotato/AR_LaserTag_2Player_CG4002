...
class GameEngine(Thread):
    def __init__(self, action_queue,viz_queue, eval_queue, from_eval_queue , phone_response_queue,shot_queue ,to_rs_queue ):
    ...
    def process_action(self,action,player_id):
        ...
        if len(action_parts) !=2:
            print_message('Game Engine', f"Invalid action format received: '{action}'")
            return False 
         
        action_type, player_id_now = action_parts 
        player_id_now = int(player_id_now)

        if player_id_now != player_id:
            return False

        if (player_id == 1 and self.p1_action_count >= 1) or (player_id == 2 and self.p2_action_count >= 1):
            print_message('Game Engine', f"Extra action received for player {player_id}")
            return False 
        
        if player_id == 1:
            self.p1_action_count +=1
        elif player_id == 2:
            self.p2_action_count +=1 
        
        if action_type == "logout" and self.game_turns_passed <= self.LOGOUT_FAILSAFE_TURNS:
            print_message('Game Engine', f'Logout detected before turn 21 for Player {player_id}, reverting to default action')
            action = f"basket:{player_id}"

        print_message('Game Engine', f"Processing action '{action}' for Player {player_id}")
        viz_format = self.process_phone_action_and_get_viz_format(action)
        phone_response = self.send_to_phone_and_wait_for_phone_response_with_retries(
            self.viz_queue, self.phone_response_queue, viz_format, max_retries=2, player_id=player_id
        )

        predicted_action = self.extract_action(action)
        if player_id == 1:
            self.P1_ai_predicted_action = predicted_action
        else:
            self.P2_ai_predicted_action = predicted_action

        prev_action = self.process_phone_response_and_return_prev_action(phone_response)
        eval_format = self.prepare_eval_server_format(player_id, prev_action)
        print("Game Engine: Putting into eval_queue to be sent to eval_server")
        self.eval_queue.put(eval_format)

        print("Game Engine: Waiting for from_eval_queue")
        updated_game_state = self.from_eval_queue.get()
        print("updated game state:", updated_game_state)

        if self.is_curr_game_state_diff_from_updated(updated_game_state):
            print("Game Engine: curr game state diff from eval game state, updating")
            self.update_current_game_state(updated_game_state)
            self.viz_queue.put(self.process_phone_action_and_get_viz_format("update_ui:1"))
        ...
        print_message('Game Engine', f"Sending info back to relay client for Player {player_id}")

        return True  # Indicate the action was processe
    ...
    def run(self):
        while True:
            self.p1_action_count = 0
            self.p2_action_count = 0
            p1_action_processed = False
            p2_action_processed = False
            
            start_time = time.time()
            while(time.time() - start_time) < 30:
                if p1_action_processed and p2_action_processed:
                    break 
                if not self.action_queue.empty():
                    try:
                            action = self.action_queue.get()
                            if action.endswith(":1") and not p1_action_processed:
                                p1_action_processed = self.process_action(action, 1)
                            elif action.endswith(":2") and not p2_action_processed:
                                p2_action_processed = self.process_action(action, 2)
                    except queue.Empty:
                        print_message('Game Engine',"Waiting for new actions")
                        pass

            if not p1_action_processed:
               print_message('Game Engine', "Timeout for Player 1, assigning default action 'basket'")
               self.process_action("basket:1", 1)
            if not p2_action_processed:
                print_message('Game Engine', "Timeout for Player 2, assigning default action 'basket'")
                self.process_action("basket:2", 2) 
            
            self.clear_queue(self.action_queue)
