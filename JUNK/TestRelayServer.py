...
class RelayServer(Thread):
    ...
    def processMessage(self, msg):
        ...
        with open("received_messages(1).log", "a") as log_file:
    ...
    def sendToRelayClient(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                game_engine_data = self.from_game_engine_queue.get(timeout=1)  #player_id: , hp: , ammo: , reload: bool (0 or 1) : enough bullets + got hit 
                if game_engine_data:
...
if __name__ == "__main__":
    try:
        relay_server = RelayServer('0.0.0.0', 6055)  # Start the server
        relay_server.start()
